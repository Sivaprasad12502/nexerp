...
type ApiResponse = {
  payoutReceipt: PayoutReceiptRow;
  businessSettings: BusinessSettingsRow | null;
  vendorEmail: string | null;
};

export default function ReceivedPayoutReceiptPage() {
  ...
  useEffect(() => {
    fetch(`/api/payout-receipts/received/${token}`)
      .then(async (r) => {
        const body = await r.json();
        if (!r.ok) throw new Error(body.error ?? "Failed to load payout receipt");
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: "Payout Receipt", url });
      } catch {
        // user cancelled
      }
    } else {
    ...
    return (
      <div className="flex min-h-[calc(100vh-3.5rem)] items-center justify-center">
        <div className="flex items-center gap-2 text-zinc-500">
          ...
          <span>Loading payout receipt…</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex min-h-[calc(100vh-3.5rem)] items-center justify-center px-4">
        <div className="max-w-sm rounded-xl border border-zinc-200 bg-white p-8 text-center shadow-sm">
          ...
          <h1 className="text-lg font-semibold text-zinc-900">Payout Receipt Unavailable</h1>
  ...
  const { payoutReceipt: receipt, businessSettings } = data;
