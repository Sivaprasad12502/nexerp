...
import type { PayoutReceiptLineInput } from "@/lib/validations/payout-receipt";

type Props = {
  open: boolean;
  onClose: () => void;
  initial?: PayoutReceiptLineInput | null;
  receiptDate: string;
  currency: string;
  onSave: (line: PayoutReceiptLineInput) => void;
};
...
export function AddPayoutRecordModal({
  open,
  onClose,
  initial,
  receiptDate,
  ...
  const [method, setMethod] = useState<PayoutReceiptLineInput["method"]>("ACCOUNT_TRANSFER");
            ...
            <FormField label={<span>Payment Method<span className="text-red-500">*</span></span>}>
              <div className="relative">
                <select
                  value={method}
                  onChange={(e) =>
                    setMethod(e.target.value as PayoutReceiptLineInput["method"])
                  }
