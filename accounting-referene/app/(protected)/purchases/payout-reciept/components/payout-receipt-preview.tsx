...
import type { PayoutReceiptPreviewData } from "@/lib/payout-receipt-preview-adapter";
    ...
    <>
      <div className="mb-6 grid grid-cols-2 gap-4">
        ...
        <div className="rounded border border-zinc-200 p-4">
          ...
          {receipt.paidToName && (
            <p className="text-sm font-semibold">{receipt.paidToName}</p>
          )}
          {receipt.paidToCountry && (
            <p className="mt-1 text-xs text-zinc-600">{receipt.paidToCountry}</p>
          )}
        </div>
      </div>
...
export function PayoutReceiptPreview({
  receipt,
  settings,
  businessSettings: bs,
  numberFormat,
