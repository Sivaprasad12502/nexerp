...
const DEBIT_NOTE_CONFIG = {
  ...
  invalidateKey: "debit-notes",
  titleFallback: "Debit Note",
  resourceLabel: "Debit Note",
  responseKey: "document",
  documentKind: "debit-note" as const,
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] bg-zinc-50 pb-10">
      ...
      <div className="px-6 py-6 sm:px-8">
        <QuotationForm
          config={DEBIT_NOTE_CONFIG}
          onCancel={() => router.push("/purchases/debit-note")}
          onSaved={(id) => router.push(`/purchases/debit-note/${id}`)}
        />
      </div>
    </div>
  );
