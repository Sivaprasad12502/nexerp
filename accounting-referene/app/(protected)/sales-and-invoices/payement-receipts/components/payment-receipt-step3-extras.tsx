...
export type PaymentReceiptExtras = {
  notes: string;
...
type Props = {
  value: PaymentReceiptExtras;
  onChange: (value: PaymentReceiptExtras) => void;
  className?: string;
};

export function PaymentReceiptStep3Extras({ value, onChange, className }: Props) {
  ...
  const patch = (partial: Partial<PaymentReceiptExtras>) =>
...
export function UnpaidInvoicesEmptyState({
  clientId,
}: {
  clientId: string;
}) {
  return (
    <div className="rounded-lg border border-zinc-200 bg-zinc-50/40 px-6 py-10 text-center">
      ...
      <p className="text-base font-semibold text-zinc-900">No unpaid Invoices found</p>
      <p className="mx-auto mt-2 max-w-md text-sm text-zinc-500">
        There are no unpaid Invoices against this client. This payment will be recorded as
        advance payment.
      </p>
      <a
        href={`/sales-and-invoices/clients-prospects/${clientId}`}
        target="_blank"
        rel="noopener noreferrer"
