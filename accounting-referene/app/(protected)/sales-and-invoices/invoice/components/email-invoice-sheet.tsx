...
import type { InvoiceRow } from "@/lib/hooks/use-invoices";

type EmailInvoiceSheetProps = {
  open: boolean;
...
function buildDefaultMessage(invoice: InvoiceRow): string {
  const amount = formatCurrency(invoice.totalAmount, invoice.currency);
  return `Hi ${invoice.billedTo},

Please find the invoice attached.

Invoice Date: ${fmt(invoice.documentDate)}
Invoice No: ${invoice.documentNumber ? `#${invoice.documentNumber}` : "Draft"}
Amount: ${amount}
...
export function EmailInvoiceSheet({
  open,
  onOpenChange,
  invoice,
}: EmailInvoiceSheetProps) {
  const qc = useQueryClient();
  const defaultMessage = buildDefaultMessage(invoice);
  const defaultSubject = `[Important] Invoice For ${invoice.billedTo}`;
  ...
  const sendMutation = useMutation({
    mutationFn: async (data: DocumentSendInput) => {
      const res = await fetch(`/api/documents/${invoice.id}/send`, {
        method: "POST",
    ...
    onSuccess: (body) => {
      toast.success(
        body.emailSent
          ...
          : "Invoice saved (email delivery unavailable)",
      );
      ...
      >
        ...
        >
          ...
          <div className="grid grid-cols-[140px_1fr] items-center gap-3 px-6 py-3">
            ...
            <Input readOnly value={invoice.billedTo} className="bg-zinc-50 text-zinc-600" />
          </div>
          ...
          <div className="flex items-start gap-2 bg-zinc-50 px-6 py-3">
            ...
            <p className="text-xs text-zinc-500">
              Invoice view link will be added to the email automatically.
            </p>
          </div>
