...
import { AddPaymentAccountModal, type PaymentAccount } from "../../documents/components/add-payment-account-modal";
...
type Props = {
  ...
  onApproved: (result?: { paymentReceiptId?: string | null }) => void;
};
  ...
  const approveMutation = useMutation({
    mutationFn: () => {
      if (payment) {
        ...
        return fetch(`/api/proforma-payments/${payment.id}`, {
          method: "PATCH",
        ...
        return fetch(`/api/proforma-invoices/${documentId}/payments`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            amountReceived: a,
    ...
    onSuccess: (data: { paymentReceiptId?: string | null }) => {
      toast.success(
        payment
          ? "Payment approved. Proforma invoice marked as Paid!"
          : "Payment recorded successfully!",
      );
      qc.invalidateQueries({ queryKey: ["proforma-invoices", documentId] });
        ...
        >
          ...
          <div className="max-h-[70vh] overflow-y-auto px-6 py-5">
            ...
            <div className="mb-5 space-y-1.5 rounded-lg bg-zinc-50 px-4 py-3 text-sm">
              <SummaryRow label="Proforma Invoice No." value={invoice.documentNumber} />
