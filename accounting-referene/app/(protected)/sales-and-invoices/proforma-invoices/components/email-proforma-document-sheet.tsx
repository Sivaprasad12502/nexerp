...
export function EmailProformaDocumentSheet({
  open,
  onOpenChange,
  document,
  documentId,
  ...
  const sendMutation = useMutation({
    mutationFn: async (data: DocumentSendInput) => {
      const res = await fetch(`/api/proforma-invoices/${documentId}/send`, {
        method: "POST",
    ...
    onSuccess: (body) => {
      toast.success(
        body.emailSent
          ? `${documentLabel} proforma invoice emailed successfully!`
