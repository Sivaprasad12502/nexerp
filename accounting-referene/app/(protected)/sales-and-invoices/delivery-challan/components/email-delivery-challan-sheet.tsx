...
import type { DeliveryChallanRow } from "@/lib/hooks/use-delivery-challans";

type EmailDeliveryChallanSheetProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  deliveryChallan: DeliveryChallanRow;
};
...
function buildDefaultMessage(challan: DeliveryChallanRow): string {
  const amount = formatCurrency(challan.challanAmount, challan.currency);
  return `Hi ${challan.issuedTo},

Please find the delivery challan attached.

Delivery Challan Date: ${fmt(challan.documentDate)}
Delivery Challan No: ${challan.documentNumber ? `#${challan.documentNumber}` : "Draft"}
Amount: ${amount}
Issued To: ${challan.issuedTo}
...
export function EmailDeliveryChallanSheet({
  open,
  onOpenChange,
  deliveryChallan,
}: EmailDeliveryChallanSheetProps) {
  const qc = useQueryClient();
  const defaultSubject = `[Important] Delivery Challan For ${deliveryChallan.issuedTo}`;
  ...
  const {
    ...
    defaultValues: {
      to: deliveryChallan.clientEmail ?? "",
      cc: [],
      replyTo: "",
      subject: defaultSubject,
      message: buildDefaultMessage(deliveryChallan),
      textType: "rich",
      getEmailStatus: true,
    },
  });
  ...
  const sendMutation = useMutation({
    mutationFn: async (data: DocumentSendInput) => {
      const res = await fetch(`/api/documents/${deliveryChallan.id}/send`, {
        method: "POST",
    ...
    onSuccess: (body) => {
      toast.success(
        body.emailSent
          ? "Delivery challan emailed successfully!"
          : "Delivery challan saved (email delivery unavailable)",
      );
      ...
      >
        ...
        >
          ...
          <div className="grid grid-cols-[140px_1fr] items-center gap-3 px-6 py-3">
            ...
            <Input readOnly value={deliveryChallan.issuedTo} className="bg-zinc-50 text-zinc-600" />
          </div>
          ...
          <div className="flex items-start gap-2 bg-zinc-50 px-6 py-3">
            ...
            <p className="text-xs text-zinc-500">
              Delivery challan view link will be added to the email automatically.
            </p>
          </div>
