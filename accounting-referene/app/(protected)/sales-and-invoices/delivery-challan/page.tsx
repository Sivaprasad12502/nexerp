...
import {
  Loader2, FileText, Eye, Pencil, Copy, MoreVertical, Columns3,
...
import { useDeliveryChallans, useDeleteDeliveryChallan, type DeliveryChallanRow } from "@/lib/hooks/use-delivery-challans";
import { EmailDeliveryChallanSheet } from "./components/email-delivery-challan-sheet";
...
function statusBadges(deliveryChallan: DeliveryChallanRow) {
  if (deliveryChallan.status.toUpperCase() === "CANCELLED") {
    return [{ label: "Cancelled", className: "bg-zinc-100 text-zinc-600 ring-zinc-200" }];
  }
  const ws = deliveryChallan.workflowStatus;
  if (ws === "SEEN") return [{ label: "Seen", className: "bg-emerald-50 text-emerald-700 ring-emerald-200" }];
  if (ws === "SENT") return [{ label: "Sent", className: "bg-orange-50 text-orange-700 ring-orange-200" }];
  return [{ label: "Created", className: "bg-blue-50 text-blue-700 ring-blue-200" }];
}

type ColumnKey =
  | "date" | "expand" | "deliveryChallan" | "issuedTo" | "challanAmount" | "status"
  | "taxRate" | "challanEmail" | "subTotal" | "amountAed" | "actions";

const DEFAULT_COLUMNS: Record<ColumnKey, boolean> = {
  date: true, expand: true, deliveryChallan: true, issuedTo: true, challanAmount: true,
  status: true, taxRate: true, challanEmail: true, subTotal: true, amountAed: true, actions: true,
};

const COLUMN_LABELS: Record<ColumnKey, string> = {
  date: "Date", expand: "Expand Line Items", deliveryChallan: "Delivery Challan", issuedTo: "Issued To",
  challanAmount: "Amount", status: "Status", taxRate: "TAX Rate",
  challanEmail: "Delivery Challan Email", subTotal: "Sub Total",
  amountAed: "Delivery Challan Amount in AED", actions: "Actions",
};
...
function LineItemsTable({ items }: { items: DeliveryChallanRow["items"] }) {
  return (
...
export default function DeliveryChallansPage() {
  ...
  const { data, isLoading, isError } = useDeliveryChallans();
  const deleteMutation = useDeleteDeliveryChallan();

  const [expanded, setExpanded] = useState<Set<string>>(new Set());
  const [columns, setColumns] = useState(DEFAULT_COLUMNS);
  const [emailDeliveryChallan, setEmailDeliveryChallan] = useState<DeliveryChallanRow | null>(null);
  ...
  const allDeliveryChallans = data?.deliveryChallans ?? [];

  const clientOptions = useMemo(
    () => Array.from(new Set(allDeliveryChallans.map((cn) => cn.issuedTo).filter(Boolean))).sort(),
    [allDeliveryChallans],
  );
  const contactOptions = useMemo(
    () => Array.from(new Set(allDeliveryChallans.map((cn) => cn.clientEmail ?? cn.issuedTo).filter(Boolean) as string[])).sort(),
    [allDeliveryChallans],
  );

  const filteredDeliveryChallans = useMemo(() => {
    let result = allDeliveryChallans;
    ...
    if (filterStatus === "created") result = result.filter((cn) => cn.workflowStatus === "CREATED");
    if (filterStatus === "seen") result = result.filter((cn) => cn.workflowStatus === "SEEN");
    if (filterStatus === "cancelled") result = result.filter((cn) => cn.status.toUpperCase() === "CANCELLED");
    if (filterStatus === "sent") result = result.filter((cn) => cn.workflowStatus === "SENT" || cn.emailSent);
    ...
    return result;
  }, [allDeliveryChallans, viewMode, filterStatus, filterClient, filterContact, filterDateStart, filterDateEnd]);

  const total = filteredDeliveryChallans.length;
  const allTotal = allDeliveryChallans.length;
  ...
  const openEmail = (deliveryChallan: DeliveryChallanRow, e?: React.MouseEvent) => { e?.stopPropagation(); setEmailDeliveryChallan(deliveryChallan); };
  const handleDelete = (deliveryChallan: DeliveryChallanRow, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!confirm(`Delete delivery challan ${deliveryChallan.documentNumber}?`)) return;
    deleteMutation.mutate(deliveryChallan.id);
  };
  ...
  const RowActions = ({ deliveryChallan }: { deliveryChallan: DeliveryChallanRow }) => (
    <div className="flex items-center gap-1">
      <button type="button" title="View" onClick={() => goToDoc(deliveryChallan.id)} className="rounded p-1.5 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700"><Eye className="size-4" /></button>
      <button type="button" title="Edit" onClick={() => router.push(`/sales-and-invoices/documents/${deliveryChallan.id}/edit`)} className="rounded p-1.5 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700"><Pencil className="size-4" /></button>
      <button type="button" title="Copy" onClick={() => navigator.clipboard.writeText(deliveryChallan.documentNumber)} className="rounded p-1.5 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700"><Copy className="size-4" /></button>
      <button type="button" title="Delete" onClick={(e) => handleDelete(deliveryChallan, e)} className="rounded p-1.5 text-zinc-400 hover:bg-zinc-100 hover:text-red-600"><Trash2 className="size-4" /></button>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={() => openEmail(deliveryChallan)}>Email Delivery Challan</DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem className="text-red-600 focus:text-red-600" onClick={(e) => handleDelete(deliveryChallan, e as unknown as React.MouseEvent)}>Delete</DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
  );
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] bg-zinc-50">
      <div className="border-b border-zinc-200 bg-white px-6 py-4 sm:px-8">
        <p className="mb-1 text-xs text-zinc-400">apporg <span className="mx-1">›</span> Delivery Challans <span className="mx-1">›</span></p>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h1 className="flex items-center gap-2 text-xl font-bold text-zinc-900">Delivery Challans <Lightbulb className="size-5 text-amber-400" /></h1>
          <div className="flex items-stretch overflow-hidden rounded-md shadow-sm">
            <button type="button" onClick={() => router.push("/sales-and-invoices/delivery-challan/new")} className="flex items-center gap-2 bg-[#e8145a] px-4 py-2 text-sm font-semibold text-white hover:bg-[#c91050]">
              <Plus className="size-4" /> Create Delivery Challan
            </button>
      ...
      {activeTab !== "overview" ? (
        ...
        <div className="px-4 py-4 sm:px-6">
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-stretch overflow-hidden rounded-md border border-zinc-200 bg-white shadow-sm">
              <select value={viewMode} onChange={(e) => setViewMode(e.target.value as ViewMode)} className="appearance-none bg-transparent py-2 pl-3 pr-8 text-sm text-zinc-700 outline-none">
                <option value="active">Active Delivery Challan</option>
            ...
            {filtersExpanded && (
              <div className="border-t border-zinc-100 px-5 pb-4 pt-4">
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  <div>
                    <label className="mb-1 block text-xs font-medium text-zinc-500">Delivery Challan Status</label>
          ...
          <CollapsibleSection title="Delivery Challan Summary" expanded={summaryExpanded} onToggle={() => setSummaryExpanded((v) => !v)}>
            Delivery challan summary metrics coming soon.
          </CollapsibleSection>
          <CollapsibleSection title="Delivery Challan Graph" expanded={graphExpanded} onToggle={() => setGraphExpanded((v) => !v)}>
            Delivery challan graph coming soon.
          </CollapsibleSection>

          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <p className="text-sm text-zinc-500">Showing <strong>1 to {total}</strong> of <strong>{allTotal} Delivery Challans</strong></p>
            <ColumnToggle />
          </div>

          <div className="overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm">
            {isLoading ? (
              <div className="flex items-center justify-center gap-2 py-20 text-zinc-500"><Loader2 className="size-5 animate-spin" /> Loading delivery challans…</div>
            ) : isError ? (
              <div className="py-20 text-center text-sm text-red-500">Failed to load delivery challans.</div>
              ...
              <div className="flex flex-col items-center gap-3 py-20 text-zinc-400">
                ...
                <p className="text-sm">{allTotal === 0 ? "No delivery challans yet. Create one to get started." : "No delivery challans match the current filters"}</p>
                {allTotal === 0 ? (
                  <button type="button" onClick={() => router.push("/sales-and-invoices/delivery-challan/new")} className="rounded-md bg-[#7438dc] px-4 py-2 text-sm font-semibold text-white hover:bg-[#6230c4]">Create Delivery Challan</button>
                ) : (
                ...
                <table className="min-w-[1800px] w-full text-sm">
                  <thead>
                    <tr>
                      ...
                      {columns.deliveryChallan && <SortTh label="Delivery Challan" />}
                      {columns.issuedTo && <th className={thClass}>Issued To</th>}
                      {columns.challanAmount && <SortTh label="Amount" right />}
                      {columns.status && <SortTh label="Status" />}
                      {columns.taxRate && <th className={thClass}>TAX Rate</th>}
                      {columns.challanEmail && <th className={thClass}>Delivery Challan Email</th>}
                      {columns.subTotal && <SortTh label="Sub Total" right />}
                      {columns.amountAed && <th className={`${thClass} text-right`}>Delivery Challan Amount in AED</th>}
                      {columns.actions && <th className={`${thClass} w-44`}>Actions</th>}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-100">
                    {filteredDeliveryChallans.map((deliveryChallan, index) => {
                      const isOpen = expanded.has(deliveryChallan.id);
                      const badges = statusBadges(deliveryChallan);
                      const fc = (n: number) => formatCurrency(n, deliveryChallan.currency);
                      return (
                        <Fragment key={deliveryChallan.id}>
                          <tr className="cursor-pointer bg-white hover:bg-zinc-50/80" onClick={() => goToDoc(deliveryChallan.id)}>
                            ...
                            {columns.date && <td className="whitespace-nowrap px-3 py-2.5 text-zinc-700">{fmt(deliveryChallan.documentDate)}</td>}
                            {columns.expand && (
                              <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                                <button type="button" onClick={() => toggleExpand(deliveryChallan.id)} className="flex size-6 items-center justify-center rounded border border-zinc-300 bg-white text-sm text-zinc-600 hover:bg-zinc-50">{isOpen ? "−" : "+"}</button>
                              </td>
                            )}
                            {columns.deliveryChallan && <td className="px-3 py-2.5"><span className="font-medium text-zinc-900">{deliveryChallan.documentNumber}</span></td>}
                            {columns.issuedTo && <td className="px-3 py-2.5 text-zinc-700">{deliveryChallan.issuedTo || "—"}</td>}
                            {columns.challanAmount && <td className="whitespace-nowrap px-3 py-2.5 text-right font-medium text-zinc-900">{fc(deliveryChallan.challanAmount)}</td>}
                            {columns.status && (
                            ...
                            {columns.taxRate && <td className="px-3 py-2.5 text-zinc-700">{deliveryChallan.taxRate}</td>}
                            {columns.challanEmail && (
                              <td className="px-3 py-2.5 text-xs" onClick={(e) => e.stopPropagation()}>
                                {deliveryChallan.emailSent ? (
                                  <span className="text-zinc-500">Sent {deliveryChallan.sentAt ? fmt(deliveryChallan.sentAt) : ""}</span>
                                ) : (
                                  <span className="text-zinc-500">Not Sent <button type="button" onClick={(e) => openEmail(deliveryChallan, e)} className="font-semibold text-[#7438dc] hover:underline">[Send]</button></span>
                                )}
                              </td>
                            )}
                            {columns.subTotal && <td className="whitespace-nowrap px-3 py-2.5 text-right text-zinc-700">{fc(deliveryChallan.subTotal)}</td>}
                            {columns.amountAed && <td className="whitespace-nowrap px-3 py-2.5 text-right font-medium text-zinc-900">{fc(deliveryChallan.totalAmount)}</td>}
                            {columns.actions && <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}><RowActions deliveryChallan={deliveryChallan} /></td>}
                          </tr>
                          {isOpen && columns.expand && (
                            <tr className="bg-zinc-50/60">
                              <td colSpan={countCols()} className="px-4 py-3"><LineItemsTable items={deliveryChallan.items} /></td>
                            </tr>
                          )}
                        </Fragment>
                      );
      ...
      {emailDeliveryChallan && (
        <EmailDeliveryChallanSheet
          open={Boolean(emailDeliveryChallan)}
          onOpenChange={(open: boolean) => !open && setEmailDeliveryChallan(null)}
          deliveryChallan={emailDeliveryChallan}
        />
      )}
    </div>
  );
