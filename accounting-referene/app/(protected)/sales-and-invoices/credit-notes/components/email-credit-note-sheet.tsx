...
import type { CreditNoteRow } from "@/lib/hooks/use-credit-notes";

type EmailCreditNoteSheetProps = {
  open: boolean;
...
function buildDefaultMessage(creditNote: CreditNoteRow): string {
  const amount = formatCurrency(creditNote.creditIssued, creditNote.currency);
  return `Hi ${creditNote.issuedTo},

Please find the credit note attached.

Credit Note Date: ${fmt(creditNote.documentDate)}
Credit Note No: ${creditNote.documentNumber ? `#${creditNote.documentNumber}` : "Draft"}
Credit Issued: ${amount}
Issued To: ${creditNote.issuedTo}
${creditNote.creditReason ? `Reason: ${creditNote.creditReason}` : ""}
...
export function EmailCreditNoteSheet({
  open,
  onOpenChange,
  creditNote,
}: EmailCreditNoteSheetProps) {
  const qc = useQueryClient();
  const defaultSubject = `[Important] Credit Note For ${creditNote.issuedTo}`;
  ...
  const {
    ...
    defaultValues: {
      ...
      message: buildDefaultMessage(creditNote),
      textType: "rich",
      getEmailStatus: true,
    },
  });
  ...
  const sendMutation = useMutation({
    mutationFn: async (data: DocumentSendInput) => {
      const res = await fetch(`/api/documents/${creditNote.id}/send`, {
        method: "POST",
    ...
    onSuccess: (body) => {
      toast.success(
        body.emailSent
          ...
          : "Credit note saved (email delivery unavailable)",
      );
      ...
      >
        ...
        >
          ...
          <div className="grid grid-cols-[140px_1fr] items-center gap-3 px-6 py-3">
            ...
            <Input readOnly value={creditNote.issuedTo} className="bg-zinc-50 text-zinc-600" />
          </div>
          ...
          <div className="flex items-start gap-2 bg-zinc-50 px-6 py-3">
            ...
            <p className="text-xs text-zinc-500">
              Credit note view link will be added to the email automatically.
            </p>
          </div>
