...
export default function PublicProformaInvoicePage() {
  ...
  const refetch = () =>
    fetch(`/api/public/proforma-invoices/${token}`)
      .then(async (r) => {
        const body = await r.json();
        if (!r.ok) throw new Error(body.error ?? "Failed to load proforma invoice");
  ...
  const paymentMutation = useMutation({
    mutationFn: () =>
      fetch(`/api/public/proforma-invoices/${token}/payments`, {
        method: "POST",
  ...
  const acceptMutation = useMutation({
    mutationFn: (payload: {
      ...
      fetch(`/api/public/proforma-invoices/${token}/accept`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    onSuccess: (result) => {
      ...
      router.push(`/sales-and-invoices/proforma-invoices/${result.document.id}`);
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const rejectMutation = useMutation({
    mutationFn: (reason: string) =>
      fetch(`/api/public/proforma-invoices/${token}/reject`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const addExpenditureMutation = useMutation({
    mutationFn: () =>
      fetch(`/api/public/proforma-invoices/${token}/add-expenditure`, {
        method: "POST",
      }).then(async (r) => {
        const body = await r.json();
        if (!r.ok)
  ...
  const rejectExpenditureMutation = useMutation({
    mutationFn: () =>
      fetch(`/api/public/proforma-invoices/${token}/reject-expenditure`, {
        method: "POST",
      }).then(async (r) => {
        const body = await r.json();
        if (!r.ok)
  ...
  const handleInvoiceNumberSubmit = () => {
    ...
    if (!trimmed) {
      toast.error("Please enter a proforma invoice number");
      return;
    }
      ...
      <div className="mx-auto max-w-4xl px-4 pt-6 sm:px-6">
        ...
        {effectiveAccepted && (
          <div className="mb-6 flex items-start gap-3 rounded-xl border border-purple-200 bg-purple-50 px-5 py-4">
            ...
            <div>
              <p className="font-semibold text-purple-800">Proforma Invoice Created</p>
              <p className="mt-0.5 text-sm text-purple-600">
                Your proforma invoice has been created in your business.
              </p>
              {(createdInvoiceId ?? invoiceDocumentId) && (
                <a
                  href={`/sales-and-invoices/proforma-invoices/${createdInvoiceId ?? invoiceDocumentId}`}
        ...
        {isRejected && !effectiveAccepted && (
          <div className="mb-6 flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-5 py-4">
            ...
            <div>
              <p className="font-semibold text-red-800">Proforma Invoice Rejected</p>
