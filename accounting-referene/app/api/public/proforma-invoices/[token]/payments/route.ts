...
export async function POST(req: NextRequest, { params }: RouteCtx) {
  ...
  const payment = await prisma.$transaction(async (tx) => {
    ...
    await notifyBusinessOwner(tx, document.businessId, {
      ...
      message: `A payment of ${document.currency} ${data.amountReceived} was recorded for proforma invoice ${document.documentNumber}. Please review and approve.`,
      entityType: "DOCUMENT",
  ...
  try {
    ...
    if (seller?.user?.email) {
      const origin =
        ...
        "";
      const approveUrl = `${origin}/sales-and-invoices/proforma-invoices/${document.id}?payment=${payment.id}`;
