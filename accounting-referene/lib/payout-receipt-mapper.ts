...
import { resolvePayoutReceiptDisplayStatus } from "@/lib/payout-receipt-utils";

type LineWithAccount = PayoutReceiptLine & {
  paymentAccount?: Pick<PaymentAccount, "id" | "displayName" | "bankName" | "accountNumber"> | null;
};

type AllocationWithDoc = PayoutReceiptAllocation & {
  document?: Pick<Document, "id" | "documentNumber" | "documentDate" | "totalAmount" | "currency" | "clientName"> | null;
  payment?: Pick<Payment, "id" | "tdsWithheld" | "transactionCharge"> | null;
};

export function mapPayoutReceiptRow(
  receipt: PayoutReceipt & {
    vendor?: Pick<Vendor, "id" | "name" | "email" | "country"> | null;
  ...
  const allocationVendorName =
    (receipt.allocations ?? []).find((a) => a.document?.clientName)?.document?.clientName ?? null;
  ...
  return {
    ...
    displayStatus: resolvePayoutReceiptDisplayStatus(
      receipt.status,
      receipt.type,
      allocations.length,
    ),
    vendorId: receipt.vendorId,
    vendorName: receipt.vendor?.name ?? allocationVendorName ?? null,
    vendorEmail: receipt.vendor?.email ?? null,
    vendorCountry: receipt.vendor?.country ?? null,
...
export type PayoutReceiptRow = ReturnType<typeof mapPayoutReceiptRow>;
