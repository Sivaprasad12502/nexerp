...
export async function settleInvoicesForReceipt(
  tx: Tx,
  params: {
    businessId: string;
    userId: string;
  ...
  for (const alloc of params.allocations) {
    ...
    if (!document) throw new Error(`INVALID_INVOICE:${alloc.documentId}`);

    const paymentStatus = parseInvoicePaymentStatus(document.settings);
    if (paymentStatus === "PAID") {
      throw new Error(`INVOICE_ALREADY_PAID:${alloc.documentId}`);
    }
...
export function sumLines(lines: PaymentReceiptCreateInput["lines"]) {
  return lines.reduce((s, l) => s + l.amountReceived, 0);
}

export function deriveReceiptStatus(
  type: PaymentReceiptCreateInput["type"],
  totalReceived: number,
  totalAllocated: number,
) {
  return computeReceiptStatus(type, totalReceived, totalAllocated);
}
