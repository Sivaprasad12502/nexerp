...
import { debitNoteCreateSchema } from "@/lib/validations/quotation";
...
function parseSettings(settings: unknown): DebitNoteSettings {
  if (typeof settings !== "object" || settings === null) return {};
  return settings as DebitNoteSettings;
}
...
export async function GET() {
  ...
  if (!ctxCan(ctx, "debit-notes", "view")) {
  ...
  const debitNotes = documents.map((doc) => {
    ...
    const debitConsumed = settings.debitConsumed ?? 0;
    const amountDue = Math.max(0, doc.totalAmount - debitConsumed);
    ...
    return {
      ...
      debitReason: settings.debitReason ?? null,
      discountOffered: settings.discountOffered ?? null,
      paymentStatus: settings.paymentStatus ?? "UNPAID",
      paymentDate: settings.paymentDate ?? null,
      emailSent: Boolean(doc.sentAt),
      acceptanceStatus: (settings.acceptanceStatus as string) ?? null,
  ...
  return NextResponse.json({ debitNotes, total: debitNotes.length });
}

export async function POST(req: NextRequest) {
  const ctx = await getRbacContext();
  if (!ctx) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!ctxCan(ctx, "debit-notes", "create")) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }
  ...
  const result = debitNoteCreateSchema.safeParse(body);
  if (!result.success) {
  ...
  try {
    const document = await prisma.$transaction(async (tx) => {
      const count = await tx.document.count({
        where: { businessId: ctx.businessId, type: "DEBIT_NOTE" },
      });
      const prefix = DOCUMENT_TYPE_PREFIX.DEBIT_NOTE;
      ...
      const doc = await tx.document.create({
        data: {
          ...
          documentDate: data.quotationDate
            ...
            : new Date(),
          validTillDate: null,
          title: ns(data.quotationTitle) ?? "Debit Note",
    ...
    if (process.env.NODE_ENV === "development") {
      console.error("[POST /api/debit-notes]", err);
    }
