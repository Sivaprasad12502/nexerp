...
export async function POST(_req: NextRequest, { params }: RouteCtx) {
  ...
  if (!recipientEmail || !sessionEmail || recipientEmail !== sessionEmail) {
    return NextResponse.json(
      {
        error:
          "Only the intended recipient can add this invoice as an expenditure. Please sign in with the email address this invoice was sent to.",
      },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    console.error("[public:invoices:add-expenditure:POST]", err);
