...
import type { PurchaseOrderRow } from "@/lib/hooks/use-purchase-orders";

// ─── Types ────────────────────────────────────────────────────────────────────

type EmailPurchaseOrderSheetProps = {
  open: boolean;
...
function buildDefaultMessage(po: PurchaseOrderRow): string {
  const amount = formatCurrency(po.totalAmount, po.currency);
  return `Hi ${po.vendorName},

Further to our discussion, please find the purchase order attached.

P.O. Date: ${fmt(po.documentDate)}
Purchase Order No: ${po.documentNumber ? `#${po.documentNumber}` : "Draft"}
Amount: ${amount}
...
export function EmailPurchaseOrderSheet({
  open,
  onOpenChange,
  purchaseOrder,
}: EmailPurchaseOrderSheetProps) {
  const qc = useQueryClient();
  const po = purchaseOrder;

  const defaultMessage = buildDefaultMessage(po);
  const defaultSubject = `[Important] Email Purchase Order For ${po.vendorName}`;
  ...
  useEffect(() => {
    if (open) {
      reset({
        ...
        replyTo: "",
        subject: defaultSubject,
        message: buildDefaultMessage(po),
        textType: "rich",
        getEmailStatus: true,
      });
    }
  ...
  const sendMutation = useMutation({
    mutationFn: async (data: DocumentSendInput) => {
      const res = await fetch(`/api/documents/${po.id}/send`, {
        method: "POST",
    ...
    onSuccess: (body) => {
      toast.success(
        body.emailSent
          ...
          : "Purchase order saved (email delivery unavailable)",
      );
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {/* <div className="mx-6 mt-4 flex items-center justify-between gap-3 rounded-lg bg-zinc-900 px-4 py-3">
          <div className="flex items-center gap-2">
            <Mail className="size-4 text-white" />
            <span className="text-sm text-white">Connect your personal and work emails!</span>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit((data) => sendMutation.mutate(data))} className="divide-y divide-zinc-100">
          ...
          <div className="grid grid-cols-[140px_1fr] items-center gap-3 px-6 py-3">
            ...
            <Input readOnly value={po.vendorName} className="bg-zinc-50 text-zinc-600" />
          </div>
          ...
          <div className="flex items-start gap-2 bg-zinc-50 px-6 py-3">
            ...
            <p className="text-xs text-zinc-500">
              Purchase order online Link will be added to the email automatically.
            </p>
          </div>
