...
import type { ProformaInvoiceRow } from "@/lib/hooks/use-proforma-invoices";

type EmailProformaSheetProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  proforma: ProformaInvoiceRow;
};
...
function buildDefaultMessage(proforma: ProformaInvoiceRow): string {
  const amount = formatCurrency(proforma.totalAmount, proforma.currency);
  return `Hi ${proforma.billedTo},

Please find the proforma invoice attached.

Proforma Invoice Date: ${fmt(proforma.documentDate)}
Proforma Invoice No: ${proforma.documentNumber ? `#${proforma.documentNumber}` : "Draft"}
Amount: ${amount}
Billed To: ${proforma.billedTo}
...
export function EmailProformaSheet({
  open,
  onOpenChange,
  proforma,
}: EmailProformaSheetProps) {
  const qc = useQueryClient();
  const defaultSubject = `[Important] Proforma Invoice For ${proforma.billedTo}`;
  ...
  const {
    ...
    defaultValues: {
      to: proforma.clientEmail ?? "",
      cc: [],
      replyTo: "",
      subject: defaultSubject,
      message: buildDefaultMessage(proforma),
      textType: "rich",
      getEmailStatus: true,
    },
  });
  ...
  const sendMutation = useMutation({
    mutationFn: async (data: DocumentSendInput) => {
      const res = await fetch(`/api/proforma-invoices/${proforma.id}/send`, {
        method: "POST",
    ...
    onSuccess: (body) => {
      toast.success(
        body.emailSent
          ? "Proforma invoice emailed successfully!"
          : "Proforma invoice saved (email delivery unavailable)",
      );
      ...
      >
        ...
        >
          ...
          <div className="grid grid-cols-[140px_1fr] items-center gap-3 px-6 py-3">
            ...
            <Input readOnly value={proforma.billedTo} className="bg-zinc-50 text-zinc-600" />
          </div>
          ...
          <div className="flex items-start gap-2 bg-zinc-50 px-6 py-3">
            ...
            <p className="text-xs text-zinc-500">
              Proforma invoice view link will be added to the email automatically.
            </p>
          </div>
