...
import { RecordNewPaymentModal } from "./components/record-new-payment-modal";
import { PaymentReceiptMoreMenu } from "./components/payment-receipt-more-menu";
import { SettledInvoicesExpandRow } from "./components/settled-invoices-expand-row";
...
export default function PaymentReceiptsPage() {
  ...
  const { data, isLoading, isError } = usePaymentReceipts({
    search,
    ...
    const rows = receipts.map((r) =>
      visibleKeys
        ...
        .map((k) => {
          ...
          if (k === "billedTo") return r.clientName ?? "";
          if (k === "amount")
    ...
    <div className="min-h-[calc(100vh-3.5rem)] bg-zinc-50 pb-10">
      <div className="border-b border-zinc-200 bg-white px-6 pb-0 pt-4 sm:px-8">
        <nav className="text-sm text-zinc-400">
          Sales &amp; Invoices <ChevronRight className="mx-0.5 inline size-3.5" />
          Payment Receipts
        </nav>
        <div className="mt-1 flex flex-wrap items-end justify-between gap-4 pb-4">
          <div className="flex items-center gap-2">
            <Lightbulb className="size-7 text-amber-400" />
            <h1 className="text-[28px] font-bold text-zinc-900">Payment Receipts</h1>
          </div>
          <button
            type="button"
        ...
        {tab === "report" ? (
          ...
          <>
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                <option value="active">Active Payment Receipt</option>
                <option value="all">All Payment Receipts</option>
              </select>
              <button
                type="button"
                onClick={downloadCSV}
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm text-zinc-500">
                ...
                <strong>{total}</strong> Payment Receipts
              </p>
              <DropdownMenu>
            ...
            <div className="overflow-hidden rounded-lg border border-zinc-200 bg-white">
              {isLoading ? (
                ...
                <p className="py-20 text-center text-sm text-zinc-400">No payment receipts yet.</p>
              ) : (
                  ...
                  <table className="min-w-[1100px] w-full text-sm">
                    <thead className="border-b border-zinc-200 bg-zinc-50 text-xs text-zinc-500">
                      <tr>
                        ...
                        {columns.expand && <th className="px-3 py-3 text-left">+ Expand Invoices</th>}
                        {columns.receipt && <th className="px-3 py-3 text-left">Payment Receipt</th>}
                        {columns.billedTo && <th className="px-3 py-3 text-left">Billed To</th>}
                        {columns.amount && <th className="px-3 py-3 text-right">Amount</th>}
                        {columns.status && <th className="px-3 py-3 text-left">Status</th>}
                        {columns.email && <th className="px-3 py-3 text-left">Payment Receipt Email</th>}
                    ...
                    <tbody className="divide-y divide-zinc-100">
                      {receipts.map((r, idx) => (
                        <ReceiptRow
                          ...
                          onView={() => router.push(`/sales-and-invoices/payement-receipts/${r.id}`)}
                          onEdit={() => router.push(`/sales-and-invoices/payement-receipts/${r.id}/edit`)}
                          onDelete={() => {
                          ...
                          onSendEmail={() =>
                            router.push(
                              `/sales-and-invoices/payement-receipts/${r.id}?email=1`,
                            )
                          }
                        />
                      ))}
      ...
      <RecordNewPaymentModal
        ...
        onSelect={(type) => {
          ...
          const q = type === "CLIENT_ADVANCE" ? "?type=client-advance" : "?type=payment-receipt";
          router.push(`/sales-and-invoices/payement-receipts/new${q}`);
        }}
      />
    </div>
  );
  ...
  return (
    <Fragment>
      <tr className="hover:bg-zinc-50/80">
        ...
        {columns.billedTo && (
          <td className="px-3 py-3">{receipt.clientName ?? "—"}</td>
        )}
      ...
      {expanded && (
        <SettledInvoicesExpandRow receipt={receipt} colSpan={colSpan} />
      )}
    </Fragment>
  );
}
