...
export default function ReceivedDeliveryChallanPage() {
  ...
  useEffect(() => {
    ...
    if (sessionStatus === "unauthenticated") {
      redirectToAuth(router, {
        callbackPath: `/sales-and-invoices/delivery-challan/received/${token}`,
      });
      return;
    }

    fetch(`/api/delivery-challans/received/${token}`)
      .then(async (r) => {
        ...
        if (!r.ok) throw new Error(body.error ?? "Failed to load delivery challan");
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: "Delivery Challan", url });
      } catch {
        // cancelled
      }
    } else {
    ...
    return (
      <div className="flex min-h-[calc(100vh-3.5rem)] items-center justify-center px-4">
        <div className="max-w-sm rounded-xl border border-zinc-200 bg-white p-8 text-center shadow-sm">
          ...
          <h1 className="text-lg font-semibold text-zinc-900">Delivery Challan Unavailable</h1>
    ...
    <div className="min-h-[calc(100vh-3.5rem)] bg-zinc-50 px-4 py-6 sm:px-8">
      <div className="mx-auto max-w-4xl">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-xl font-bold text-zinc-900">Delivery Challan</h1>
